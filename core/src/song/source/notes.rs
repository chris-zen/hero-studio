...
impl NotesClip {
  ...
  pub fn add_notes(&mut self, notes: Vec<Note>) -> &mut Self {
    notes.iter().for_each(|note| { self.add_note(*note); });
    self
  }
  ...
  pub fn notes_range<'a>(&'a self, range_start: TicksTime, range_end: TicksTime) -> impl Iterator<Item=Note> + 'a {
    self.events
      ...
      .flat_map(move |(tick, tick_events)| {
        tick_events.iter().flat_map(move |event| match *event {
          ...
          NoteEvent::NoteEnd { key, velocity, start } => Some(Note {
            key,
            velocity,
            start,
            length: *tick - start
  ...
  pub fn events_range<'a>(&'a self, start: TicksTime, end: TicksTime) -> impl Iterator<Item=&'a NoteEvent> + 'a {
    self.events
      .range(start..end)
      .flat_map(|(_tick, tick_events)| {
        tick_events.iter().map(move |event| event)
      })
  }
  ...
  fn remove_note_event(&mut self, tick: TicksTime, event: &NoteEvent) {
    if let Some(tick_events) = self.events.get_mut(&tick) {
      if let Some(index) = tick_events.iter().position(|prev_event| prev_event == event) {
...
mod test {
  ...
  pub fn notes_clip_add_note() {
    ...
    let note1 = Note { key: 24, velocity: 1.0, start: TicksTime::new(1), length: TicksTime::new(2) };
    let note2 = Note { key: 25, velocity: 1.0, start: TicksTime::new(2), length: TicksTime::new(2) };
    let note3 = Note { key: 25, velocity: 1.0, start: TicksTime::new(2), length: TicksTime::new(2) };
    clip.add_note(note1);
    clip.add_note(note2);
    clip.add_note(note3);

    let mut expected_events: BTreeMap<TicksTime, NoteEvents> = BTreeMap::new();
    expected_events.insert(TicksTime::new(1), vec![
      NoteEvent::NoteStart { key: 24, velocity: 1.0, end: TicksTime::new(3) }]);
    expected_events.insert(TicksTime::new(2), vec![
      NoteEvent::NoteStart { key: 25, velocity: 1.0, end: TicksTime::new(4) },
      NoteEvent::NoteStart { key: 25, velocity: 1.0, end: TicksTime::new(4) }]);
    expected_events.insert(TicksTime::new(3), vec![
      NoteEvent::NoteEnd { key: 24, velocity: 1.0, start: TicksTime::new(1) }]);
    expected_events.insert(TicksTime::new(4), vec![
      NoteEvent::NoteEnd { key: 25, velocity: 1.0, start: TicksTime::new(2) },
      NoteEvent::NoteEnd { key: 25, velocity: 1.0, start: TicksTime::new(2) }]);
  ...
  pub fn notes_clip_notes_range() {
    let mut clip = NotesClip::new();
    let note1 = Note { key: 21, velocity: 1.0, start: TicksTime::new(0), length: TicksTime::new(1) };
    let note2 = Note { key: 22, velocity: 1.0, start: TicksTime::new(1), length: TicksTime::new(2) };
    let note3 = Note { key: 23, velocity: 1.0, start: TicksTime::new(2), length: TicksTime::new(2) };
    let note4 = Note { key: 24, velocity: 1.0, start: TicksTime::new(1), length: TicksTime::new(4) };
    let note5 = Note { key: 24, velocity: 1.0, start: TicksTime::new(4), length: TicksTime::new(3) };
    clip.add_notes(vec![note1, note2, note3, note4, note5]);

    let range_result: Vec<Note> = clip.notes_range(TicksTime::new(2), TicksTime::new(5)).collect();
  ...
  pub fn notes_clip_events_range() {
    ...
    let event1 = NoteEvent::NoteStart { key: 23, velocity: 1.0, end: TicksTime::new(4) };
    let event2 = NoteEvent::NoteEnd { key: 22, velocity: 1.0, start: TicksTime::new(1) };
    let event3 = NoteEvent::NoteEnd { key: 23, velocity: 1.0, start: TicksTime::new(2) };
    let event4 = NoteEvent::NoteStart { key: 24, velocity: 1.0, end: TicksTime::new(7) };
    let expected_events = vec![&event1, &event2, &event3, &event4];

    let range_result: Vec<&NoteEvent> = clip.events_range(TicksTime::new(2), TicksTime::new(5)).collect();
