...
impl Encoder {
  ...
  pub fn encode(msg: &Message, out: &mut [u8]) {
    match msg {
      Message::NoteOff { channel, key, velocity } => out[..3]
        .copy_from_slice(&[status_and_channel(0b1000, channel), u7(key), u7(velocity)]),
      Message::NoteOn { channel, key, velocity } => out[..3]
        .copy_from_slice(&[status_and_channel(0b1001, channel), u7(key), u7(velocity)]),
...
mod test {
  ...
  pub fn note_off() {
    assert_encoding(
      &Message::NoteOff { channel: 1, key: 65, velocity: 120 },
  ...
  pub fn note_on() {
    assert_encoding(
      &Message::NoteOn { channel: 1, key: 65, velocity: 120 },
